...
## DEC-0025 · 가계부 탭 상태 유지(sessionStorage) + 날짜 상세 모달 중앙 배치·테마 통일
- **날짜**: 2026-06-24
- **결정**: (1) 가계부 일자별/달력 `viewMode`를 `sessionStorage('ledgerViewMode')`에 저장/복원한다. (2) 달력 날짜 상세 모달을 하단 시트 → **화면 중앙** 배치로 변경한다. (3) 모달과 수입/지출/합계 박스 테두리를 그림판 테마 손그림 유틸 `.paint-box`로 통일한다.
- **이유**:
  - 거래 수정(`TransactionFormView`)은 별도 라우트라 저장/닫기 후 돌아오면 `LedgerView`가 재마운트되어 `viewMode`가 기본값 `list`로 초기화됨 → 달력에서 들어갔는데 일자별 목록으로 돌아오는 문제. 라우트 query로 넘기면 진입 경로마다 챙겨야 하므로, **`LedgerView` 한 곳에서 끝나는 sessionStorage**를 택해 폼 등 다른 화면을 건드리지 않음.
  - 모달은 중앙이 시선 집중에 자연스럽고, 테두리는 별도 CSS 없이 기존 `.paint-box` 컨벤션(classic 테마 무효·paint 테마 손그림 wobble)을 재사용해 테마 일관성 확보.
- **영향**:
  - `LedgerView.vue` 단일 파일(프론트 한정, 백엔드·API 무영향).
...
- **결정**: `notification`을 예산 임계치 전용에서 **유형 3종**으로 확장한다. `type` 컬럼 추가(`BUDGET`/`DRAW`/`REPORT`), `weekly_budget_id`·`threshold`를 **NULL 허용**으로 완화, `REPORT`용 `report_year`·`report_month` 추가.
  - **BUDGET**(예산 사용): 기존 실시간 트리거(지출 등록 시)에 더해, **조회 시 '지난주'(오늘 이전 끝난 가장 최근 주)를 평가해 최고 임계 1건**을 결산으로 생성(`evaluateExpense(userId, 지난주날짜)` 재사용 → `maxSent`+unique로 중복 차단). 시드로 '6월 첫주'를 하드코딩하던 알림 행은 제거하고 전부 동적 생성으로 전환. 문구는 실시간/지난주 양쪽에 쓰이므로 "이번 주"를 빼고 "주간 예산…"으로 중립화.
  - **DRAW**(옷 뽑기 기회): 끝난 주(end_date<오늘) + 절약 성공(지출≤예산) + 미결정(reward_status NULL/PENDING)인 주마다 1건.
