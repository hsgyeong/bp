...
async function loadList() {
  loading.value = true
  try {
    const res = await listNotifications(0)   // 안읽음만 (읽으면 목록에서 사라짐)
...
// 한 건 클릭 -> 읽음 처리 + 배지 감소 + 목록에서 제거(다시 안 뜸) -> 유형별 화면 이동
async function onItemClick(n) {
  if (n.isRead === 0) {
    await markRead(n.id)
    count.value = Math.max(0, count.value - 1)
  }
  items.value = items.value.filter((it) => it.id !== n.id)   // 읽은 알림은 목록에서 제거
  goByType(n)
}
...
/* 부모(.noti)에 transform이 걸려 position:fixed가 .noti 기준이 되므로,
   화면 전체보다 크게 잡아 어디를 눌러도 닫히게 한다 */
.backdrop {
  position: fixed;
...
/* 안 읽음 배경(.item.unread)이 position:relative라 패널 손그림 테두리(::before)를 덮음
   → 테두리를 항목 위로 올려 가려지지 않게 (테두리 내부는 투명이라 내용은 그대로 보임) */
:root[data-theme="paint"] .panel::before { z-index: 2; }
