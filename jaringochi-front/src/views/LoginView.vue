  ...
  <section class="login-page">                        <!-- section: 화면의 한 구역-->
    <div class="login-body">
      <div class="mascot-wrap" aria-hidden="true">    <!-- aria-hidden: 화면의 읽기 도구가 읽지 않게 함 -->
        ...
        <GulbiMascot v-if="theme === 'paint'" mood="hello" :size="104" />
        <svg v-else class="gulbi" viewBox="0 0 120 120">     <!-- svg: 벡터 그림을 그리는 태그 -->
          <defs>
...
/* ── paint(그림판) 테마 보정 ── */
/* 매단 줄 숨김(흑백 굴비 마스코트로 교체) */
:root[data-theme="paint"] .string { display: none; }
/* 말풍선: 크림 배경/갈색 글씨 → 흑백. 테두리는 ::after 로 손그림(::before 는 말풍선 꼬리) */
:root[data-theme="paint"] .mascot-speech {
  background: var(--card);
  border-color: transparent;
  color: var(--ink);
}
:root[data-theme="paint"] .mascot-speech span { color: var(--mute); }
:root[data-theme="paint"] .mascot-speech::before {
  background: var(--card);
  border-left-color: var(--ink);
  border-top-color: var(--ink);
}
:root[data-theme="paint"] .mascot-speech::after {
  content: "";
  position: absolute;
  inset: 0;
