...
const dateInput = ref(null)            // 날짜 input 참조 (박스 클릭 시 달력 열기)
function openDatePicker() {
    // showPicker는 사용자 클릭 제스처 안에서 호출해야 동작 (미지원 브라우저는 기존 아이콘 클릭으로 폴백)
  ...
  <section  v-else class="transaction-form-view">
    ...
    <form v-else class="form-body" @submit.prevent="saveTransaction">
      ...
      <div class="field-block">
        ...
        <label class="date-field paint-box" @click="openDatePicker">
          <span>{{ formatDateLabel(date) }}</span>
          <small>
            <i v-if="theme === 'paint'" class="ti ti-calendar" aria-hidden="true"></i>
            <template v-else>📅</template>
          </small>
          <input id="date" ref="dateInput" v-model="date" type="date" />
        </label>
      </div>
