...
CREATE TABLE `monthly_report` (
  ...
  -- 통계 스냅샷 (생성 시점 고정)
  `total_expense`    DECIMAL(12,2) NULL                       COMMENT '이번 달 총 지출',
  `prev_expense`     DECIMAL(12,2) NULL                       COMMENT '전월 총 지출',
  `diff_ratio`       DECIMAL(7,2)  NULL                       COMMENT '전월 대비 %, 전월=0이면 NULL',
  ...
  `replied_at`       DATETIME      NULL                       COMMENT '응답 시각(NULL=아직 안 씀)',
